	...
	public double incidenciaAcumulada(String pais) {
		return locator.getJsonStream()
				...
				.max((it1,it2)->Utilidades.convertirTextoFecha(it1.getFecha())
						        .compareTo(Utilidades.convertirTextoFecha(it2.getFecha())))   // compara las 2 fechas con el compareTo y devuelve un optional
				.orElse(new Item())
				.getIncidencia();
	...
	public int  casosTotales() {
		return locator.getJsonStream()
				...
				.collect(Collectors.groupingBy( it -> it.getNombrePais() ,    //  Agrupamos por pais, obteniendo un map clave pais y el valor el ITEM de la ultima semana, 
												Collectors.maxBy((it1,it2)->Utilidades.convertirTextoFecha(it1.getFecha())
